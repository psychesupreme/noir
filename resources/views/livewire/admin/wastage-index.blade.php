                ...
                <tbody class="divide-y divide-neutral-900/60 text-sm font-light">
                    @forelse ($logs as $log)
                        <tr class="hover:bg-neutral-950/40 transition-colors">
                            {{-- Product --}}
                            <td class="p-6">
                                <span class="text-white font-normal block font-mono uppercase">{{ $log->product->name }}</span>
                            ...
                            <td class="p-6">
                                @if (in_array($log->reason, ['Spoilage', 'Expired']))
                                    <span class="inline-block text-[9px] tracking-wider uppercase font-mono px-2 py-0.5 rounded-sm bg-rose-950/40 text-rose-400 border border-rose-900/30">
                                        {{ $log->reason }}
                                    </span>
                                @elseif ($log->reason === 'Damaged')
                                    <span class="inline-block text-[9px] tracking-wider uppercase font-mono px-2 py-0.5 rounded-sm bg-amber-950/40 text-amber-400 border border-amber-900/30">
                                        {{ $log->reason }}
                                    </span>
                                ...
                                @else
                                    <span class="inline-block text-[9px] tracking-wider uppercase font-mono px-2 py-0.5 rounded-sm bg-neutral-905 bg-neutral-900 text-neutral-400 border border-neutral-800">
                                        {{ $log->reason }}
                                    </span>
                                @endif
                            </td>
    ...
    @if($showLogModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                {{-- Modal Panel --}}
                <div class="inline-block align-middle bg-[#0B0B0E] border border-neutral-900 rounded-sm text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
                    <div class="px-6 py-6">
