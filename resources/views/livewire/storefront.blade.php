...
>
    ...
    <style>
        ...
        /* Premium content layering stagger entrance animations */
        @keyframes layer-fade-in-up {
            0% {
                opacity: 0;
        ...
        .animate-layer-1 {
            animation: layer-fade-in-up 0.9s cubic-bezier(0.16, 1, 0.3, 1) both;
        }
        .animate-layer-2 {
            animation: layer-fade-in-up 0.9s cubic-bezier(0.16, 1, 0.3, 1) both;
        ...
        >
            ...
            >
                ...
                <div class="absolute inset-0 bg-cover bg-center select-none"
                     :class="{
                          'filter brightness-75 contrast-125 saturate-[0.8]': theme === 'dark',
                          'filter brightness-90 sepia-[0.15] saturate-[0.95]': theme === 'light'
                     }"
            ...
            <button 
                ...
                :class="theme === 'light' ? 'border-neutral-200 bg-white/70 text-black hover:bg-neutral-100' : 'border-neutral-800/40 bg-black/60 text-white hover:bg-neutral-900'"
                ...
                <div class="flex items-center space-x-6 text-sm tracking-widest">
                    <template x-for="(numeral, index) in ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X'].slice(0, slidesCount)" :key="index">
                        <button @click="activeSlide = index; resetTimer();"
                                :class="activeSlide === index ? (theme === 'light' ? 'text-black font-bold' : 'text-white font-bold') : 'text-neutral-500 hover:text-neutral-300'"
            ...
            <aside :class="theme === 'light' ? 'border-neutral-200 bg-white/50' : 'border-neutral-900/60 bg-[#0C0C0E]/40'" class="hidden lg:block w-60 shrink-0 sticky top-24 p-4.5 border rounded-[28px] backdrop-blur-md space-y-4 text-left transition-all duration-500 shadow-sm z-10 max-h-[calc(100vh-110px)] overflow-y-auto scrollbar-none theme-section">
                ...
                <div class="border-t border-neutral-500/10 pt-3">
                    <div :class="theme === 'light' ? 'bg-neutral-100/60 text-black border-neutral-200' : 'bg-neutral-900/40 text-neutral-300 border-neutral-800/80'" class="p-3 rounded-[18px] border space-y-3">
                                ...
                                @foreach($cartItems as $item)
                                    <div class="flex items-center justify-between gap-1.5 text-[10px] pb-1 border-b border-neutral-500/5">
                                        <div class="truncate flex-1">
                                            <span :class="theme === 'light' ? 'text-neutral-800' : 'text-white'" class="font-sans font-normal block truncate uppercase text-[9px] tracking-wider">{{ $item['product']->name }}</span>
                        ...
                        <button @click="drawerOpen = true" :class="theme === 'light' ? 'bg-black text-white hover:bg-neutral-800' : 'bg-white text-black hover:bg-neutral-200'" class="w-full mt-1.5 py-2 rounded-xl text-[9px] font-sans font-light uppercase tracking-widest font-semibold transition-all cursor-pointer text-center block shadow-sm">
                            Open Workspace
                        </button>
                    </div>
                </div>
                        ...
                        @if($product->category === 'specializtion' || $product->category === 'specialization' || $product->category === 'specializations')
                            ...
                            >
                                ...
                                <div @click="quickViewProduct = {{ toJsObject($product) }}; quickViewSize = 'standard'; quickViewOpen = true;"
                                    ...
                                    >
                                        <svg class="w-4.5 h-4.5 {{ $inWishlist ? 'fill-current text-rose-500' : 'fill-none stroke-current text-[#C5A880]' }}" viewBox="0 0 24 24" stroke-width="1.5">
                                ...
                                <div class="flex-1 pl-3 flex flex-col justify-between overflow-hidden">
                                    ...
                                    <div class="space-y-2 mt-1">
                                        ...
                                        <div :class="theme === 'light' ? 'text-neutral-600' : 'text-neutral-400'" class="flex items-center space-x-2.5 text-[10px] font-outfit uppercase">
                                ...
                                <div @click="quickViewProduct = {{ toJsObject($product) }}; quickViewSize = 'standard'; quickViewOpen = true;"
                                    ...
                                    <div class="aspect-[4/5] rounded-t-[180px] rounded-b-[16px] relative overflow-hidden bg-neutral-950/5">
                                        ...
                                        @elseif($product->original_price && $product->original_price > $product->price)
                                            <span class="absolute top-3 left-3 z-20 bg-rose-600 text-white text-[9px] font-outfit uppercase font-bold tracking-widest px-2.5 py-0.5 rounded-full shadow-md">
                                        ...
                                        <button 
                                            ...
                                            @click.stop=""
                                            wire:click="toggleWishlist({{ $product->id }})" 
                                            class="absolute top-3 right-3 z-20 w-9 h-9 rounded-full flex items-center justify-center bg-[#0B0B0D]/60 border border-white/5 text-[#C5A880] hover:scale-110 hover:bg-neutral-900 transition-all cursor-pointer shadow-md"
                                            title="{{ $inWishlist ? 'Remove from Wishlist' : 'Add to Wishlist' }}"
                                        >
                                            <svg class="w-4.5 h-4.5 {{ $inWishlist ? 'fill-current text-rose-500' : 'fill-none stroke-current text-[#C5A880]' }}" viewBox="0 0 24 24" stroke-width="1.5">
                                                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                                ...
                                <div class="px-2 pt-4 pb-2 flex-1 flex flex-col justify-between">
                                    ...
                                    <div class="mt-4 space-y-3">
                                        ...
                                        <div class="max-h-none opacity-100 lg:max-h-0 lg:opacity-0 lg:group-hover:max-h-32 lg:group-hover:opacity-100 transition-all duration-500 ease-in-out overflow-hidden space-y-3">
                                            ...
                                            <div :class="theme === 'light' ? 'text-neutral-600' : 'text-neutral-400'" class="flex items-center justify-between border-t border-neutral-500/10 pt-2.5 text-[10px] font-outfit uppercase tracking-widest">
                                                <span>Share:</span>
                                                <div class="flex items-center space-x-3">
                                                    <!-- Instagram Icon -->
                                                    <a href="https://instagram.com" target="_blank" rel="noopener" class="hover:text-pink-500 transition-colors" title="Instagram">
                                                    ...
                                                    <a href="https://twitter.com/intent/tweet?text=Loving%20the%20premium%20{{ urlencode($product->name) }}%20curation%2520from%20@NoirAndBloom.%20Check%20it%20out:&url={{ urlencode(url('/')) }}" 
                                                       target="_blank" rel="noopener" :class="theme === 'light' ? 'hover:text-black' : 'hover:text-white'" class="transition-colors" title="Share on X">
                                                        <svg class="w-3 h-3 fill-current" viewBox="0 0 24 24">
                                                            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                                                        </svg>
                                                    </a>
                    ...
                    @if($hasMore)
                        ...
                        >
                            ...
                            <div x-show="loading" class="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
                                <template x-for="i in 3" :key="i">
                                    <div class="animate-pulse rounded-2xl overflow-hidden"
                                         :class="theme === 'light' ? 'bg-neutral-100' : 'bg-neutral-900/40'">
                                        <div class="aspect-[4/5] rounded-t-2xl"
                                             :class="theme === 'light' ? 'bg-neutral-200' : 'bg-neutral-800'"></div>
                            ...
                            <div x-show="loading" class="flex items-center space-x-2 py-2">
                                <svg class="animate-spin h-3.5 w-3.5" :class="theme === 'light' ? 'text-neutral-400' : 'text-neutral-600'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                                ...
                                <span class="text-[10px] font-mono uppercase tracking-[0.3em]" :class="theme === 'light' ? 'text-neutral-400' : 'text-neutral-600'">Curating more...</span>
                            </div>
                        </div>
                    @else
                        ...
                        <div class="flex flex-col items-center space-y-2 py-4 animate-hero-fade">
                            <div class="h-[1px] w-32 bg-gradient-to-r from-transparent to-transparent mb-1"
                                 :class="{
                                     'via-[#C5A880]': theme === 'dark',
                                     'via-emerald-600': theme === 'light',
                                 }"></div>
                        ...
                        <div class="flex-1 overflow-y-auto p-5 space-y-5 max-h-none md:max-h-[calc(85vh-180px)] scrollbar-none text-xs">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-1 gap-2">
                                    ...
                                    <button type="button" @click="$wire.delivery_type = 'secret'" :class="$wire.delivery_type === 'secret' ? (theme === 'light' ? 'border-[#B59A7A] bg-[#B59A7A]/5 text-[#B59A7A] font-semibold' : 'border-[#C5A880] bg-[#C5A880]/15 text-[#C5A880] font-semibold') : (theme === 'light' ? 'border-neutral-200 text-neutral-500 hover:border-neutral-350' : 'border-neutral-850 text-neutral-400 hover:border-neutral-700')" class="p-3 border rounded-xl text-left flex justify-between items-center transition-all cursor-pointer">
                                    ...
                                    <button type="button" @click="$wire.delivery_type = 'concierge'" :class="$wire.delivery_type === 'concierge' ? (theme === 'light' ? 'border-[#B59A7A] bg-[#B59A7A]/15 text-[#B59A7A] font-semibold' : 'border-[#C5A880] bg-[#C5A880]/25 text-[#C5A880] font-semibold') : (theme === 'light' ? 'border-neutral-200 text-neutral-500 hover:border-neutral-350' : 'border-neutral-850 text-neutral-400 hover:border-neutral-700')" class="p-3 border rounded-xl text-left flex justify-between items-center transition-all cursor-pointer">
                            ...
                            <div class="space-y-2" wire:ignore>
                                    ...
                                    x-data="{
                                        ...
                                        initMap() {
                                            ...
                                            const setup = () => {
                                                ...
                                                let goldIcon = L.divIcon({
                                                    html: `<div class="w-6 h-6 flex items-center justify-center rounded-full bg-neutral-900 border border-[#C5A880] shadow-lg shadow-black/50">
                                                             <div class="w-2.5 h-2.5 rounded-full bg-[#C5A880]"></div>
                                                           </div>`,
                                ...
                                >
                                    ...
                                    <div :class="isFullscreen ? 'fixed inset-0 w-screen h-screen z-[9999] bg-black/60 flex items-center justify-center' : 'relative w-full h-44 md:h-64 rounded-xl overflow-hidden border border-neutral-500/10 z-10'">
                                        ...
                                        <div x-show="isFullscreen" 
                                             ...
                                             class="absolute top-4 md:top-6 left-1/2 -translate-x-1/2 w-[calc(100vw-32px)] sm:w-[480px] p-3 md:p-4 border rounded-2xl flex flex-col items-center justify-center gap-2 md:gap-3 z-[1010]"
                                         >
                                         ...
                                         <div id="checkout-map" class="w-full h-full" :class="isFullscreen ? 'absolute inset-0 z-0' : 'min-h-[176px] md:min-h-[256px]'"></div>
                            ...
                            @if($paymentStatus === 'idle')
                                ...
                                <div class="block md:hidden space-y-3 p-4 border rounded-2xl shadow-sm text-left transition-all duration-300 animate-hero-fade"
                                    ...
                                    <div class="flex items-center justify-between pb-2 border-b border-neutral-500/10">
                                        ...
                                        <span class="font-mono text-xs font-bold" :class="theme === 'light' ? 'text-neutral-900' : 'text-[#C5A880]'">{{ number_format($cartTotal + $service_fee + $delivery_fee) }} KSH</span>
                                    </div>
            ...
            <div :class="theme === 'light' ? 'bg-[#FAF7F0]/40' : 'bg-black/25'" class="hidden md:flex w-full md:w-2/5 flex flex-col justify-between overflow-y-auto md:overflow-hidden h-auto md:h-full">
                <div class="flex-1 overflow-y-auto p-5 space-y-5 max-h-none md:max-h-[calc(85vh-100px)] scrollbar-none text-xs">
                    ...
                    <div class="space-y-1.5 text-neutral-500 font-mono text-[11px] pt-2 border-t border-neutral-500/10">
                        <div class="flex justify-between">
                            ...
                            <span>{{ number_format((int)round(($cartTotal + $service_fee + $delivery_fee) / 1.16)) }} KSH</span>
                        </div>
                        <div class="flex justify-between">
                            <span>VAT Compliance (16%):</span>
                            <span>{{ number_format(($cartTotal + $service_fee + $delivery_fee) - (int)round(($cartTotal + $service_fee + $delivery_fee) / 1.16)) }} KSH</span>
                        </div>
                        @if($delivery_fee > 0)
                            <div class="flex justify-between text-amber-600 dark:text-amber-400">
                                <span>Delivery Fee ({{ $closest_hub }}{{ $delivery_distance_km > 0 ? ' ' . $delivery_distance_km . 'km' : '' }}):</span>
                                <span>+ {{ number_format($delivery_fee) }} KSH</span>
                            </div>
                        @endif
                        @if($service_fee > 0)
                ...
                <div :class="theme === 'light' ? 'border-neutral-200 bg-neutral-50/80' : 'border-neutral-900 bg-black/40'" class="p-5 border-t space-y-4 shrink-0 mt-auto">
                    <div class="flex justify-between items-center">
                        ...
                        <span :class="theme === 'light' ? 'text-black' : 'text-amber-500'" class="text-2xl font-mono font-bold tracking-tight">{{ number_format($cartTotal + $service_fee + $delivery_fee) }} KSH</span>
                    </div>
                </div>
            </div>
        </div>
...
<!-- Floating Chat Widget -->
    <div x-show="!drawerOpen" x-transition class="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
        ...
        <button 
            ...
            :class="theme === 'light' ? 'bg-[#FAF7F0]/85 text-black border-neutral-250/60 shadow-md backdrop-blur-md' : 'bg-[#0A0A0A]/85 text-white border-neutral-800 shadow-2xl backdrop-blur-md'"
