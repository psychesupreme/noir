<div 
    ...
    :class="{
        ...
        'bg-[#06130F] text-[#ECFDF5]': theme === 'jade'
    }"
...
>
    ...
    <div x-show="theme !== 'champagne'" class="absolute inset-0 overflow-hidden pointer-events-none z-0" style="display: none;">
        ...
        <div 
            :class="{
                'bg-[#C5A880]/6 animate-onyx-glow': theme === 'onyx',
                'bg-[#B59A7A]/8 animate-champagne-glow': theme === 'champagne',
                'bg-emerald-500/6 animate-jade-glow': theme === 'jade'
            }"
            class="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full blur-[120px] transition-all duration-1000"
        ></div>
        <!-- Blob 2 -->
        <div 
            :class="{
                'bg-purple-950/4 animate-onyx-glow': theme === 'onyx',
                'bg-amber-450/4 animate-champagne-glow': theme === 'champagne',
                'bg-[#D4AF37]/6 animate-jade-glow': theme === 'jade'
            }"
            class="absolute top-1/2 right-1/4 w-[600px] h-[600px] rounded-full blur-[140px] transition-all duration-1000"
        ></div>
    </div>
    ...
    <header 
        :class="{
            ...
            'border-[#142E24]/60 bg-[#06130F]/80 shadow-2xl': theme === 'jade'
        }"
    ...
    >
        <div class="max-w-7xl w-full mx-auto px-6 flex items-center justify-between gap-8">
            <a href="/" class="shrink-0 flex items-baseline space-x-2 animate-nav-item select-none cursor-pointer" style="animation-delay: 100ms;">
                ...
                <h1 :class="theme === 'champagne' ? 'text-black' : 'text-white'" class="text-sm font-semibold uppercase tracking-[0.3em] transition-colors">NOIR & BLOOM</h1>
            </a>
            ...
            <div class="flex items-center space-x-6 text-[10px] font-mono uppercase tracking-widest text-neutral-400">
                <!-- Theme Switcher Button (Cycles onyx -> champagne -> jade) -->
                <button @click="theme = theme === 'onyx' ? 'champagne' : (theme === 'champagne' ? 'jade' : 'onyx')" 
                        class="hover:text-neutral-200 transition-colors cursor-pointer select-none relative w-8 h-8 flex items-center justify-center border border-neutral-500/10 rounded-full bg-neutral-500/5 animate-nav-item"
                        style="animation-delay: 300ms;"
                        title="Cycle Theme (Onyx / Champagne / Jade)"
                >
                    <!-- Onyx (Dark): crescent moon -->
                    <svg x-show="theme === 'onyx'" class="w-4 h-4 stroke-current fill-none" viewBox="0 0 24 24" stroke-width="1.5">
                        <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
                    </svg>
                    <!-- Champagne (Light): sun -->
                    <svg x-show="theme === 'champagne'" class="w-4 h-4 stroke-current fill-none" viewBox="0 0 24 24" stroke-width="1.5" style="display: none;">
                    ...
                    <!-- Jade (Lotus/Grid): grid flower pattern -->
                    <svg x-show="theme === 'jade'" class="w-4 h-4 stroke-current fill-none" viewBox="0 0 24 24" stroke-width="1.5" style="display: none;">
                        <path d="M12 3v18M3 12h18M12 3c3 4.5 3 13.5 0 18M12 3c-3 4.5-3 13.5 0 18M3 12c4.5-3 13.5-3 18 0M3 12c4.5 3 13.5 3 18 0" />
                    </svg>
                </button>
                ...
                >
                    ...
                    @if($cartCount > 0)
                        <span class="absolute -top-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-[#C5A880] text-black text-[8px] font-bold font-sans tracking-none shadow-[0_0_8px_rgba(197,168,128,0.5)]">
                            {{ $cartCount }}
                        </span>
                    @endif
                </button>

                <!-- Profile Portal Sign In / User svg button -->
                @auth
                ...
                @else
                    <!-- Profile Portal - Logged Out (Show sign-in/arrow entry SVG) -->
                    <button @click="accountPanelOpen = true" 
                            class="hover:text-neutral-300 transition-colors cursor-pointer select-none w-8 h-8 flex items-center justify-center border border-neutral-500/10 rounded-full bg-neutral-500/5 animate-nav-item"
                            style="animation-delay: 500ms;"
    ...
    <footer 
        :class="{
            ...
            'border-[#142E24]/40 bg-[#0E1512] text-neutral-300': theme === 'jade'
        }"
    ...
    >
        <div class="max-w-7xl w-full mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-left">
            ...
            <div class="space-y-4">
                <div class="flex items-baseline space-x-2">
                    ...
                    <h4 :class="theme === 'champagne' ? 'text-black' : 'text-white'" class="text-xs font-semibold uppercase tracking-[0.3em] transition-colors">NOIR & BLOOM</h4>
                </div>
            ...
            <div class="space-y-4">
                <h5 :class="theme === 'champagne' ? 'text-neutral-900' : 'text-neutral-300'" class="text-[10px] font-mono uppercase tracking-[0.2em] font-semibold">The Showroom</h5>
            ...
            <!-- Col 3: Hours & Support -->
            <div class="space-y-4">
                <h5 :class="theme === 'champagne' ? 'text-neutral-900' : 'text-neutral-300'" class="text-[10px] font-mono uppercase tracking-[0.2em] font-semibold">Concierge Dispatch</h5>
                <ul class="space-y-2 text-[11px] font-light">
            ...
            <div class="space-y-4">
                <h5 :class="theme === 'champagne' ? 'text-neutral-900' : 'text-neutral-300'" class="text-[10px] font-mono uppercase tracking-[0.2em] font-semibold">The Atelier Bulletin</h5>
                ...
                <div class="flex items-center space-x-2 pt-1">
                    <input 
                        ...
                        class="flex-1 text-[11px] px-3.5 py-2.5 border rounded-xl focus:outline-none transition-all"
                    >
                    <button 
                        :class="theme === 'champagne' ? 'bg-neutral-950 text-white hover:bg-black' : 'bg-white text-black hover:bg-neutral-200'"
                        class="px-4 py-2.5 text-[9px] font-mono uppercase tracking-wider font-semibold rounded-full transition-all"
                    >
                        Join
                    </button>
                </div>
        ...
        <div :class="theme === 'champagne' ? 'border-neutral-200/60 text-neutral-500' : 'border-neutral-900 text-neutral-600'" class="max-w-7xl w-full mx-auto border-t mt-12 pt-8 flex flex-col md:flex-row justify-between items-center text-[10px] font-mono uppercase tracking-wider gap-4">
    ...
    <div 
        ...
        x-transition:enter="transition transform ease-in-out duration-500" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
        x-transition:leave="transition transform ease-in-out duration-500" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full"
        :class="theme === 'champagne' ? 'bg-white border-neutral-200 text-neutral-900' : 'bg-[#0F0F12] border-neutral-900 text-white'"
        class="fixed inset-y-0 right-0 w-full sm:w-[480px] border-l shadow-2xl z-50 flex flex-col justify-between text-left transition-colors duration-500 h-full rounded-l-3xl"
        style="display: none;"
    >
        ...
        <div x-show="!checkoutMode && !@json($orderSubmitted)" class="flex-1 flex flex-col justify-between overflow-hidden">
            <div class="flex-1 overflow-y-auto p-5 space-y-5 max-h-[calc(100vh-220px)] scrollbar-none">
                @forelse($cartItems as $item)
                    <div :class="theme === 'champagne' ? 'border-neutral-100' : 'border-neutral-900/60'" class="flex items-center justify-between space-x-4 border-b pb-4 text-xs">
                        <div class="flex-1 space-y-0.5">
                            <h4 :class="theme === 'champagne' ? 'text-neutral-800' : 'text-white'" class="font-normal">{{ $item['product']->name }}</h4>
            ...
            @if(count($cartItems) > 0)
                <div :class="theme === 'champagne' ? 'border-neutral-200 bg-neutral-50/60' : 'border-neutral-900 bg-black/40'" class="p-5 border-t space-y-4 shrink-0">
                    ...
                    <button @click="checkoutMode = true" class="w-full bg-black text-white dark:bg-white dark:text-black text-[10px] font-semibold tracking-[0.2em] uppercase py-4 hover:opacity-90 transition-all cursor-pointer rounded-full">
        ...
        <div x-show="checkoutMode && !@json($orderSubmitted)" class="flex-1 flex flex-col justify-between overflow-hidden" style="display: none;">
            <form wire:submit.prevent="submitCurationRequest" class="flex-1 flex flex-col justify-between overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5 max-h-[calc(100vh-210px)] scrollbar-none text-xs">
                    ...
                    <div :class="theme === 'champagne' ? 'bg-neutral-100 border-neutral-200' : 'bg-[#0A0A0A] border-neutral-900'" class="p-1 border rounded-full grid grid-cols-2 text-center text-[9px] font-mono uppercase tracking-wider shadow-inner">
                    ...
                    <div x-show="!$wire.is_gift" x-transition>
                        <div :class="theme === 'champagne' ? 'bg-neutral-100 text-black border-neutral-200' : 'bg-neutral-900/60 text-neutral-400 border-neutral-900'" class="p-3.5 border rounded-xl text-[11px] font-light space-y-1.5">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            <button type="button" wire:click="$set('delivery_type', 'standard')" :class="delivery_type === 'standard' ? 'border-black dark:border-white bg-neutral-500/5 font-medium shadow-sm' : 'border-neutral-800 text-neutral-400'" class="p-3 border rounded-xl text-left flex justify-between items-center transition-all cursor-pointer">
                            ...
                            <button type="button" wire:click="$set('delivery_type', 'secret')" :class="delivery_type === 'secret' ? 'border-amber-600 bg-amber-950/10 text-amber-500 font-medium' : 'border-neutral-800 text-neutral-400'" class="p-3 border rounded-xl text-left flex justify-between items-center transition-all cursor-pointer">
                            ...
                            <button type="button" wire:click="$set('delivery_type', 'concierge')" :class="delivery_type === 'concierge' ? 'border-emerald-600 bg-emerald-950/10 text-emerald-500 font-medium' : 'border-neutral-800 text-neutral-400'" class="p-3 border rounded-xl text-left flex justify-between items-center transition-all cursor-pointer">
                ...
                <div :class="theme === 'champagne' ? 'border-neutral-200 bg-neutral-50/60' : 'border-neutral-900 bg-black/40'" class="p-5 border-t space-y-4 shrink-0 text-xs">
                    ...
                    <button 
                        ...
                        class="w-full bg-black text-white dark:bg-white dark:text-black text-[10px] font-semibold tracking-[0.2em] uppercase py-4 disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer flex items-center justify-center space-x-2 rounded-full"
                    >
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5 max-h-[calc(100vh-200px)] scrollbar-none w-full text-xs">
                    <div class="space-y-6 max-w-sm mx-auto w-full pt-4">
                        ...
                        <div class="space-y-1 text-center">
                            <h4 :class="theme === 'champagne' ? 'text-neutral-900' : 'text-white'" class="text-xs uppercase tracking-[0.2em] font-medium">Dispatch Mapped</h4>
                        ...
                        <div :class="theme === 'champagne' ? 'bg-neutral-50 border-neutral-200 text-black' : 'bg-[#0A0A0A]/80 border-neutral-900 text-neutral-300'" class="p-4 border rounded-xl text-[11px] font-light space-y-1.5">
                        ...
                        <div :class="theme === 'champagne' ? 'bg-white border-neutral-200' : 'bg-[#0A0A0A] border-neutral-900'" class="space-y-3 p-4 border rounded-2xl shadow-2xl" x-data="{ mpesaProcessing: false }">
    ...
    <div x-show="!drawerOpen" x-transition class="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
        ...
        <div x-show="chatOpen" style="display: none;" x-transition class="mt-3 w-80 md:w-96 h-[420px] bg-[#0F0F0F] border border-neutral-800 rounded-3xl shadow-2xl flex flex-col justify-between overflow-hidden">
            ...
            <form wire:submit.prevent="sendChatMessage" class="p-3 border-t border-neutral-800 bg-[#0A0A0A] flex items-center gap-2">
                <input type="text" wire:model="chatMessage" placeholder="Ask Aura about arrangements, branches, points..." class="flex-1 bg-[#141414] border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-neutral-700 font-light">
                <button type="submit" class="bg-white text-black font-mono text-[9px] uppercase font-bold px-3.5 py-2 rounded-full cursor-pointer hover:bg-neutral-200 transition-colors">Ask</button>
            </form>
        </div>
    </div>
</div>