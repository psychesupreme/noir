        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            @if (session()->has('error'))
                <div x-data="{ show: true }"
                     ...
                     class="bg-red-950/20 border border-red-900/50 p-4 rounded-2xl text-xs text-red-200 font-mono"
                >
                    {{ session('error') }}
                </div>
            @endif
                    ...
                    @foreach($availableStems as $stem)
                        ...
                        >
                            ...
                            <div class="mt-4 flex justify-between items-center border-t border-neutral-800/10 pt-3">
                                ...
                                <!-- Quantity adjuster pill with Optimistic 0ms Alpine UI state -->
                                <div @click.stop x-data="{ qty: {{ $selectedStems[$stem->id] ?? 0 }} }" x-init="$watch('$wire.selectedStems.{{ $stem->id }}', val => qty = val || 0)" class="flex items-center space-x-3 bg-black/20 rounded-full p-1 border border-neutral-800/40 text-xs">
                                    <button 
                                        type="button"
                                        @click.stop="if ({{ $stem->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($stem->name) }} is currently out of stock.'); } else if (qty > 0) { qty--; $wire.adjustStemQuantity({{ $stem->id }}, -1); viewMode = 'arrangement'; }"
                                    ...
                                    >-</button>
                                    <span class="font-mono font-medium min-w-[16px] text-center" x-text="qty"></span>
                                    <button 
                                        type="button"
                                        @click.stop="if ({{ $stem->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($stem->name) }} is currently out of stock.'); } else { qty++; $wire.adjustStemQuantity({{ $stem->id }}, 1); viewMode = 'arrangement'; }"
            ...
            <section id="section-wrapping" class="space-y-6 scroll-mt-28">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach($availableWrappings as $wrap)
                        <div 
                            x-data="{ shake: false }"
                            @click="if ({{ $wrap->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($wrap->name) }} is currently out of stock.'); } else { selectedWrappingId = (selectedWrappingId == {{ $wrap->id }}) ? null : {{ $wrap->id }}; $wire.selectWrapping({{ $wrap->id }}); viewMode = 'presentation'; }"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-neutral-800/15">
                    ...
                    @if($glitterProduct)
                        <div 
                            @click="hasGlitter = !hasGlitter; $wire.toggleGlitter();"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($availableRibbons as $ribbon)
                                ...
                                <button 
                                    type="button"
                                    @click="selectedRibbonId = (selectedRibbonId == {{ $ribbon->id }}) ? null : {{ $ribbon->id }}; $wire.selectRibbon({{ $ribbon->id }});"
            ...
            <section id="section-scent" class="space-y-6 scroll-mt-28">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    @foreach($availableMists as $mist)
                        <div 
                            x-data="{ shake: false }"
                            @click="if ({{ $mist->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($mist->name) }} is currently out of stock.'); } else { selectedMistId = (selectedMistId == {{ $mist->id }}) ? null : {{ $mist->id }}; $wire.selectMist({{ $mist->id }}); }"
                        ...
                        @foreach($availableWines as $wine)
                            ...
                            >
                                ...
                                <div class="mt-4 flex justify-between items-center border-t border-neutral-800/10 pt-3">
                                    ...
                                    <div @click.stop x-data="{ qty: {{ $selectedGifts[$wine->id] ?? 0 }} }" x-init="$watch('$wire.selectedGifts.{{ $wine->id }}', val => qty = val || 0)" class="flex items-center space-x-3 bg-black/20 rounded-full p-1 border border-neutral-800/40 text-xs">
                                        <button 
                                            type="button"
                                            @click.stop="if ({{ $wine->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($wine->name) }} is currently out of stock.'); } else if (qty > 0) { qty--; $wire.adjustGiftQuantity({{ $wine->id }}, -1); }"
                                            class="w-6 h-6 flex items-center justify-center rounded-full text-neutral-450 hover:text-white hover:bg-neutral-850 transition cursor-pointer select-none"
                                        >-</button>
                                        <span class="font-mono font-medium min-w-[16px] text-center" x-text="qty"></span>
                                        <button 
                                            type="button"
                                            @click.stop="if ({{ $wine->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($wine->name) }} is currently out of stock.'); } else { qty++; $wire.adjustGiftQuantity({{ $wine->id }}, 1); }"
                                            class="w-6 h-6 flex items-center justify-center rounded-full text-neutral-450 hover:text-white hover:bg-neutral-850 transition cursor-pointer select-none"
                                        >+</button>
                                    </div>
                                </div>
                        ...
                        @foreach($availableChocolates as $choc)
                            ...
                            >
                                ...
                                <div class="mt-4 flex justify-between items-center border-t border-neutral-800/10 pt-3">
                                    ...
                                    <div @click.stop x-data="{ qty: {{ $selectedGifts[$choc->id] ?? 0 }} }" x-init="$watch('$wire.selectedGifts.{{ $choc->id }}', val => qty = val || 0)" class="flex items-center space-x-3 bg-black/20 rounded-full p-1 border border-neutral-800/40 text-xs">
                                        <button 
                                            type="button"
                                            @click.stop="if ({{ $choc->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($choc->name) }} is currently out of stock.'); } else if (qty > 0) { qty--; $wire.adjustGiftQuantity({{ $choc->id }}, -1); }"
                                            class="w-6 h-6 flex items-center justify-center rounded-full text-neutral-450 hover:text-white hover:bg-neutral-850 transition cursor-pointer select-none"
                                        >-</button>
                                        <span class="font-mono font-medium min-w-[16px] text-center" x-text="qty"></span>
                                        <button 
                                            type="button"
                                            @click.stop="if ({{ $choc->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($choc->name) }} is currently out of stock.'); } else { qty++; $wire.adjustGiftQuantity({{ $choc->id }}, 1); }"
                                            class="w-6 h-6 flex items-center justify-center rounded-full text-neutral-450 hover:text-white hover:bg-neutral-850 transition cursor-pointer select-none"
                                        >+</button>
                                    </div>
                                </div>
                        ...
                        @foreach($availableJewelry as $jewel)
                            ...
                            >
                                ...
                                <div class="mt-4 flex justify-between items-center border-t border-neutral-800/10 pt-3">
                                    ...
                                    <div @click.stop x-data="{ qty: {{ $selectedGifts[$jewel->id] ?? 0 }} }" x-init="$watch('$wire.selectedGifts.{{ $jewel->id }}', val => qty = val || 0)" class="flex items-center space-x-3 bg-black/20 rounded-full p-1 border border-neutral-800/40 text-xs">
                                        <button 
                                            type="button"
                                            @click.stop="if ({{ $jewel->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($jewel->name) }} is currently out of stock.'); } else if (qty > 0) { qty--; $wire.adjustGiftQuantity({{ $jewel->id }}, -1); }"
                                            class="w-6 h-6 flex items-center justify-center rounded-full text-neutral-450 hover:text-white hover:bg-neutral-850 transition cursor-pointer select-none"
                                        >-</button>
                                        <span class="font-mono font-medium min-w-[16px] text-center" x-text="qty"></span>
                                        <button 
                                            type="button"
                                            @click.stop="if ({{ $jewel->stock }} <= 0) { shake = true; setTimeout(() => shake = false, 500); $dispatch('curation-error', '{{ addslashes($jewel->name) }} is currently out of stock.'); } else { qty++; $wire.adjustGiftQuantity({{ $jewel->id }}, 1); }"
                ...
                @if($cardProduct)
                    ...
                    >
                        <div class="flex items-center justify-between">
                            ...
                            <button 
                                type="button" 
                                @click="hasCard = !hasCard; $wire.toggleCard(); if(hasCard) { viewMode = 'note'; }"
            ...
            Alpine.data('curationStudio', function() {
                return {
                    ...
                    init() {
                        ...
                        @auth
                            const pref = '{{ auth()->user()->settings["preferred_theme"] ?? "" }}';
                            this.theme = (pref === 'onyx' || pref === 'dark') ? 'dark' : ((pref === 'light') ? 'light' : ((storedTheme === 'onyx' || storedTheme === 'dark') ? 'dark' : 'light'));
                        @else
                            this.theme = (storedTheme === 'onyx' || storedTheme === 'dark') ? 'dark' : 'light';
                        @endauth
                        var self = this;
        ...
        <div class="p-6 space-y-8 flex flex-col h-full justify-between">
            <div class="space-y-6">
                ...
                <nav class="space-y-4 text-xs font-mono uppercase tracking-widest font-semibold">
                    ...
                    <a href="{{ route('curate') }}" class="block py-2 hover:text-[#C5A880] transition-colors text-[#C5A880]">Curation Studio</a>
