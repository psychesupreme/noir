...
class Storefront extends Component
{
    ...
    // Delivery upsells & distance fee
    public string $delivery_type = 'standard'; 
    public int $service_fee = 0;
    public int $delivery_fee = 0;
    public float $delivery_distance_km = 0.0;
    public string $closest_hub = 'Nairobi CBD';
    ...
    {
        if (empty(trim($this->delivery_address))) {
            $this->delivery_fee = 0;
            $this->delivery_distance_km = 0.0;
            return;
        }

        // Try to parse lat, lng from delivery_address string (e.g. "-1.286389, 36.817222 (Nairobi CBD)")
        if (!preg_match('/^\s*(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)/', $this->delivery_address, $matches)) {
            // Default rate if string is plain text address without coordinates
            $this->delivery_fee = 300;
            $this->delivery_distance_km = 0.0;
            $this->closest_hub = strtolower($this->region) === 'kiambu' ? 'Kiambu Tigoni' : 'Nairobi CBD';
            return;
        }

        $userLat = (float) $matches[1];
        $userLng = (float) $matches[2];

        // Hub coordinates (Nairobi CBD & Kiambu Tigoni)
        $hubs = [
            'Nairobi CBD' => ['lat' => -1.286389, 'lng' => 36.817222],
            'Kiambu Tigoni' => ['lat' => -1.1444, 'lng' => 36.6853],
        ];

        $minDistance = null;
        $closestHubName = 'Nairobi CBD';

        foreach ($hubs as $hubName => $hubCoords) {
            $dist = $this->haversineGreatCircleDistance($userLat, $userLng, $hubCoords['lat'], $hubCoords['lng']);
            if ($minDistance === null || $dist < $minDistance) {
                $minDistance = $dist;
                $closestHubName = $hubName;
            }
        }

        $this->delivery_distance_km = round($minDistance ?? 0.0, 1);
        $this->closest_hub = $closestHubName;

        // Dynamic distance fee calculation:
        // Base rate: KSh 300 for first 5km
        // Then KSh 35 per km up to 50km
        // Over 50km: KSh 1,800 capped rate
        if ($this->delivery_distance_km <= 5.0) {
            $this->delivery_fee = 300;
        } elseif ($this->delivery_distance_km <= 50.0) {
            $this->delivery_fee = (int) (300 + ceil(($this->delivery_distance_km - 5.0) * 35));
        } else {
            $this->delivery_fee = 1800;
        }
    }

    private function haversineGreatCircleDistance(float $latitudeFrom, float $longitudeFrom, float $latitudeTo, float $longitudeTo, float $earthRadius = 6371.0): float
    {
    ...
    public function submitCurationRequest(): void
    {
        ...
        $this->calculateDeliveryFee();
        $grandTotal = $totalAmount + $this->service_fee + $this->delivery_fee;
        ...
        $orderId = DB::transaction(function () use ($itemsToAttach, $grandTotal) {
            ...
            $order = Order::create([
                ...
                'service_fee_amount'   => $this->service_fee + $this->delivery_fee,
                'status'               => 'pending',
    ...
    public function render()
    {
        ...
        $query = Product::with('occasions')
            ...
            ->withAvg('reviews', 'freshness_rating')
        ...
        $cacheKeyProducts = 'storefront_products_' . md5(serialize([
        ...
        $products = \App\Services\StorefrontCacheService::remember($cacheKeyProducts, 600, function() use ($query, $limit) {
            return $query->take($limit)->get()->map(function($product) {
        ...
        $cacheKeyTotal = 'storefront_total_count_' . md5(serialize([$this->selectedCategory, $this->search]));
        $totalCount = \App\Services\StorefrontCacheService::remember($cacheKeyTotal, 600, fn() => $totalCountQuery->count());
        ...
        $cacheKeySuggestions = 'storefront_suggestions_' . md5($this->search);
        $suggestions = \Illuminate\Support\Facades\Cache::remember($cacheKeySuggestions, 600, fn() => $suggestionQuery->withCount('orders')->orderBy('orders_count', 'desc')->limit(5)->get());

        $offerProducts = \Illuminate\Support\Facades\Cache::remember('storefront_offer_products', 600, function() {
            return Product::where('category', '!=', 'specializtion')
                ->where('stock', '>', 0)
                ->withAvg('reviews', 'rating')
                ->withAvg('reviews', 'quality_rating')
                ->withAvg('reviews', 'freshness_rating')
