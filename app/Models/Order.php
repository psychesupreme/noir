                ...
                if ($order->client && $order->client->user_id) {
                    ...
                    switch($newStatus) {
                        ...
                        case 'shipping':
                            ...
                            // Dispatch Africa's Talking SMS
                            try {
                                $smsService = app(\App\Services\AfricasTalkingService::class);
                                $targetPhone = $order->is_gift && !empty($order->recipient_phone)
                                    ? $order->recipient_phone
                                    : ($order->client?->phone ?? null);

                                if ($targetPhone) {
                                    $smsText = "Noir & Bloom Atelier: Your luxury curation #NB-ORD-{$order->id} is now EN ROUTE! Our concierge courier is delivering your arrangement with temperature control.";
                                    $smsService->sendSms($targetPhone, $smsText);
                                }
                            } catch (\Throwable $smsErr) {
                                \Illuminate\Support\Facades\Log::error("Failed to send shipping SMS for Order #{$order->id}: " . $smsErr->getMessage());
                            }
                            break;
                        case 'delivered':
